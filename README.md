...
# Betmma Spells: Adds 11 "Spell" cards (WIP)
...
The element of sequence can be the following: Face cards, Numbered cards, Each suit and Each rank. There is "negative element" which means if the card doesn't match the element the count increases, for example "negative Spade" means "not Spade" and increases the count if current scoring card is not Spade.						
...
Currently spells appear in shop, and fusion spells only appear if you have fused it during this run.
