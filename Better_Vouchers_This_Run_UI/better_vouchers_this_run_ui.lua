...
function G.UIDEF.used_vouchers()
  ...
  for k, v in ipairs(keys_used) do
    if #G.your_collection >= PAIRS_PER_ROW*ROWS_PER_PAGE then break end
    if next(v) then
      if #G.your_collection%PAIRS_PER_ROW == 0 then
        table.insert(voucher_table_rows, {n=G.UIT.R, config={align = "cm", padding = 0, no_fill = true}, nodes=voucher_tables})
        voucher_tables = {}
      end
      G.your_collection[#G.your_collection + 1] = CardArea(G.ROOM.T.x + 0.2*G.ROOM.T.w/2, G.ROOM.T.h, (#v == 1 and 1 or 1.33)*G.CARD_W, 1.07*G.CARD_H, {card_limit = 2, type = 'voucher', highlight_limit = 0})
      for kk, vv in ipairs(v) do
        local center = G.P_CENTERS[vv.key]
        local card = Card(G.your_collection[#G.your_collection].T.x + G.your_collection[#G.your_collection].T.w/2, G.your_collection[#G.your_collection].T.y, G.CARD_W, G.CARD_H, nil, center, {bypass_discovery_center=true,bypass_discovery_ui=true,bypass_lock=true})
        card.ability.order = vv.order
        card:start_materialize(nil, silent)
        silent = true
        G.your_collection[#G.your_collection]:emplace(card)
      end
      table.insert(voucher_tables, {n=G.UIT.C, config={align = "cm", padding = 0, no_fill = true}, nodes={{n=G.UIT.O, config={object = G.your_collection[#G.your_collection]}}}})
    end
  end
  if next(voucher_tables) then table.insert(voucher_table_rows, {n=G.UIT.R, config={align = "cm", padding = 0, no_fill = true}, nodes=voucher_tables}) end
...
G.FUNCS.used_voucher_page = function(args)
  ...
  for i = 1, #G.your_collection do
    local pair = keys_used[i + (args.cycle_config.current_option - 1)*(PAIRS_PER_ROW*ROWS_PER_PAGE)]
    if not pair then break end
    for j = 1, #pair do
      local center = pair[j]
      if not center then break end
      local card = Card(G.your_collection[i].T.x + G.your_collection[i].T.w/2, G.your_collection[i].T.y, G.CARD_W, G.CARD_H, nil, center, {bypass_discovery_center=true,bypass_discovery_ui=true,bypass_lock=true})
      card.ability.order = pair[j].order
      card:start_materialize(nil, silent)
      silent = true
